...
aipl_error_t aipl_color_convert_bgr888_to_yuv_planar(const void* input,
                                                       uint32_t pitch,
                                                       uint32_t width,
                                                       uint32_t height,
                                                       uint8_t* y_ptr,
...
aipl_error_t aipl_color_convert_bgr888_to_yuv_semi_planar(const void* input,
                                                            uint32_t pitch,
                                                            uint32_t width,
                                                            uint32_t height,
                                                            uint8_t* y_ptr,
...
aipl_error_t aipl_color_convert_yuv_planar_to_bgr888(const uint8_t* y_ptr,
                                                     const uint8_t* u_ptr,
                                                     const uint8_t* v_ptr,
                                                     void* output,
                                                     uint32_t pitch,
...
aipl_error_t aipl_color_convert_yuv_semi_planar_to_bgr888(const uint8_t* y_ptr,
                                                          const uint8_t* u_ptr,
                                                          const uint8_t* v_ptr,
                                                          void* output,
                                                          uint32_t pitch,
...
aipl_error_t aipl_color_convert_yuv_packed_to_bgr888(const uint8_t* y_ptr,
                                                     const uint8_t* u_ptr,
                                                     const uint8_t* v_ptr,
                                                     void* output,
                                                     uint32_t pitch,
                                               ...
                                               uint32_t height)
{
    ...
    return aipl_color_convert_bgr888_to_yuv_planar(input, pitch,
                                                   width, height,
                                                   y_ptr, u_ptr, v_ptr);
}
#endif
                                               ...
                                               uint32_t height)
{
    ...
    for (uint32_t i = 0; i < height; ++i)
    {
        ...
        while (cnt > 0)
        {
            ...
            aipl_mve_rgb_x16_t pix;
            aipl_mve_load_bgr888_offset_16px(&pix, src, 2, tail_p);
...
aipl_error_t aipl_color_convert_bgr888_to_i400(const void* input,
                                               ...
                                               uint32_t height)
{
    return aipl_color_convert_bgr888_to_alpha8(input, output, pitch,
                                                 width, height);
}
#endif
                                               ...
                                               uint32_t height)
{
    ...
    return aipl_color_convert_bgr888_to_yuv_semi_planar(input, pitch,
                                                        width, height,
                                                        y_ptr, u_ptr, v_ptr);
}
#endif
                                               ...
                                               uint32_t height)
{
    ...
    return aipl_color_convert_yuv_planar_to_bgr888(y_ptr, u_ptr, v_ptr,
                                                   output, pitch,
                                                   width, height);
}
#endif
                                                 ...
                                                 uint32_t height)
{
    ...
    for (uint32_t i = 0; i < height; ++i)
    {
        ...
        {
            aipl_bgr888_px_t px;
            aipl_yuv_to_bgr888(&px, *(y_src++), *u_src, *v_src);
            dst->a = 0xff;
            dst->r = px.r;
            dst->g = px.g;
            dst->b = px.b;
                                               ...
                                               uint32_t height)
{
    ...
    for (uint32_t i = 0; i < height; ++i)
    {
        ...
        while (cnt > 0)
        {
            ...
            aipl_mve_store_bgr888_offset_16px(dst, &pix, 2, tail_p);

            y = vldrbq_gather_offset_z(y_src + 1, AIPL_2_BYTE_OFFSETS_U8, tail_p);

            aipl_mve_convert_yuv_to_rgb_x16(&pix, &y, &u, &v);

            aipl_mve_store_bgr888_offset_16px(dst + 3, &pix, 2, tail_p);
    ...
    for (uint32_t i = 0; i < height; ++i)
    {
        ...
        for (uint32_t j = 0; j < width; j += 2)
        {
            aipl_yuv_to_bgr888(dst++, *(y_src++), *u_src, *v_src);
                                               ...
                                               uint32_t height)
{
    ...
    return aipl_color_convert_yuv_semi_planar_to_bgr888(y_ptr, u_ptr, v_ptr,
                                                        output, pitch,
                                                        width, height);
}
#endif
                                               ...
                                               uint32_t height)
{
    ...
    return aipl_color_convert_yuv_packed_to_bgr888(y_ptr, u_ptr, v_ptr,
                                                   output, pitch,
                                                   width, height);
}
#endif
    ...
    switch (format)
    {
        ...
        case AIPL_COLOR_RGB565:
            return aipl_color_convert_i400_to_rgb565(input, output,
                                                     ...
                                                     width, height);
#endif
#if (AIPL_CONVERT_I400 & TO_BGR888 | AIPL_CONVERT_ALPHA8 & TO_BGR888)
        case AIPL_COLOR_BGR888:
                                                       ...
                                                       uint8_t* v_ptr)
{
    ...
    for (uint32_t i = 0; i < height; i += 2)
    {
        const aipl_bgr888_px_t* src0 = src_ptr + i * pitch;
        const aipl_bgr888_px_t* src1 = src0 + pitch;
                                                       ...
                                                       uint32_t height)
{
    ...
    for (uint32_t i = 0; i < height; i += 2)
    {
        ...
        {
            aipl_bgr888_px_t px;
            aipl_yuv_to_bgr888(&px, *(y_src0++), *u_src, *v_src);
            dst0->a = 0xff;
            dst0->r = px.r;
            dst0->g = px.g;
            dst0->b = px.b;
            ...
            aipl_yuv_to_bgr888(&px, *(y_src1++), *u_src, *v_src);
            dst1->a = 0xff;
            dst1->r = px.r;
            dst1->g = px.g;
            dst1->b = px.b;
                                                       ...
                                                       uint32_t height)
{
    ...
    for (uint32_t i = 0; i < height; ++i)
    {
        ...
        for (uint32_t j = 0; j < width; j += 2)
        {
            ...
            aipl_yuv_to_bgr888(&px, *(y_src + 2), *u_src, *v_src);
            dst->a = 0xff;
            dst->r = px.r;
            dst->g = px.g;
            dst->b = px.b;
                                                     ...
                                                     uint32_t height)
{
    ...
    for (uint32_t i = 0; i < height; i += 2)
    {
        ...
        while (cnt > 0)
        {
            ...
            aipl_mve_store_bgr888_offset_16px(dst0, &pix, 2, tail_p);

            y = vldrbq_gather_offset_z(y_src0 + 1, AIPL_2_BYTE_OFFSETS_U8, tail_p);

            aipl_mve_convert_yuv_to_rgb_x16(&pix, &y, &u, &v);

            aipl_mve_store_bgr888_offset_16px(dst0 + 3, &pix, 2, tail_p);

            y = vldrbq_gather_offset_z(y_src1, AIPL_2_BYTE_OFFSETS_U8, tail_p);

            aipl_mve_convert_yuv_to_rgb_x16(&pix, &y, &u, &v);

            aipl_mve_store_bgr888_offset_16px(dst1, &pix, 2, tail_p);

            y = vldrbq_gather_offset_z(y_src1 + 1, AIPL_2_BYTE_OFFSETS_U8, tail_p);

            aipl_mve_convert_yuv_to_rgb_x16(&pix, &y, &u, &v);

            aipl_mve_store_bgr888_offset_16px(dst1 + 3, &pix, 2, tail_p);
    ...
    for (uint32_t i = 0; i < height; i += 2)
    {
        ...
        const uint8_t* v_src = v_ptr + i * pitch / 4;
        const uint8_t* u_src = u_ptr + i * pitch / 4;
        aipl_bgr888_px_t* dst0 = dst_ptr + i * pitch;
        aipl_bgr888_px_t* dst1 = dst0 + pitch;

        for (uint32_t j = 0; j < width; j += 2)
        {
            aipl_yuv_to_bgr888(dst0++, *(y_src0++), *u_src, *v_src);

            aipl_yuv_to_bgr888(dst0++, *(y_src0++), *u_src, *v_src);

            aipl_yuv_to_bgr888(dst1++, *(y_src1++), *u_src, *v_src);
